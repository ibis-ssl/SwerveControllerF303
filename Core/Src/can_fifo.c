...
static inline void crit_enter(uint32_t * pm)
{
  *pm = __get_PRIMASK();
  __disable_irq();
}
...
static int queue_is_empty(void) { return (q_head == q_tail) && (q[q_tail].used == 0); }

static int queue_is_full(void) { return (q_head == q_tail) && (q[q_tail].used == 1); }
